...
impl WeatherForecaster {
    ...
    fn parse_openmeteo_current(&self, json: &Value) -> Result<CurrentWeather> {
        ...
        let sunrise_time = daily["sunrise"].as_array()
            .and_then(|arr| arr.first())
            .and_then(|v| v.as_str())
            .unwrap_or_default();
        
        let sunset_time = daily["sunset"].as_array()
            .and_then(|arr| arr.first())
            .and_then(|v| v.as_str())
            .unwrap_or_default();
        ...
        let sunset = match DateTime::parse_from_rfc3339(sunset_time) {
            Ok(dt) => dt.with_timezone(&Utc),
            Err(_) => timestamp.checked_add_signed(Duration::hours(12)).unwrap_or(timestamp), // Fallback to 12 hours later
        };
    ...
    fn parse_openmeteo_hourly(&self, json: &Value) -> Result<Vec<HourlyForecast>> {
        ...
        // Get time array
        let times = hourly["time"].as_array().ok_or_else(|| anyhow!("Missing time array"))?;
        let temps = hourly["temperature_2m"].as_array().ok_or_else(|| anyhow!("Missing temperature data"))?;
        let feels_like = hourly["apparent_temperature"].as_array().ok_or_else(|| anyhow!("Missing apparent temperature data"))?;
        let humidity = hourly["relative_humidity_2m"].as_array().ok_or_else(|| anyhow!("Missing humidity data"))?;
        let pressure = hourly["surface_pressure"].as_array().ok_or_else(|| anyhow!("Missing pressure data"))?;
        let wind_speed = hourly["wind_speed_10m"].as_array().ok_or_else(|| anyhow!("Missing wind speed data"))?;
        let wind_direction = hourly["wind_direction_10m"].as_array().ok_or_else(|| anyhow!("Missing wind direction data"))?;
        let clouds = hourly["cloud_cover"].as_array().ok_or_else(|| anyhow!("Missing cloud cover data"))?;
        let empty_vec_pop = Vec::new();
        let pop = hourly["precipitation_probability"].as_array().unwrap_or(&empty_vec_pop);
        let weather_codes = hourly["weather_code"].as_array().ok_or_else(|| anyhow!("Missing weather code data"))?;
        ...
        for i in 0..times.len().min(48) { // Limit to 48 hours (2 days)
            ...
            let wind_dir = wind_direction.get(i).and_then(|v| v.as_f64()).unwrap_or(0.0) as u16;
    ...
    fn parse_openmeteo_daily(&self, json: &Value) -> Result<Vec<DailyForecast>> {
        ...
        // Get date array
        let dates = daily["time"].as_array().ok_or_else(|| anyhow!("Missing date array"))?;
        let weather_codes = daily["weather_code"].as_array().ok_or_else(|| anyhow!("Missing weather code data"))?;
        let temp_max = daily["temperature_2m_max"].as_array().ok_or_else(|| anyhow!("Missing max temperature data"))?;
        let temp_min = daily["temperature_2m_min"].as_array().ok_or_else(|| anyhow!("Missing min temperature data"))?;
        let feels_max = daily["apparent_temperature_max"].as_array().ok_or_else(|| anyhow!("Missing max feels like data"))?;
        let feels_min = daily["apparent_temperature_min"].as_array().ok_or_else(|| anyhow!("Missing min feels like data"))?;
        let empty_vec_precip_sum = Vec::new();
        let _precip_sum = daily["precipitation_sum"].as_array().unwrap_or(&empty_vec_precip_sum);
        let wind_speed = daily["wind_speed_10m_max"].as_array().ok_or_else(|| anyhow!("Missing wind speed data"))?;
        let wind_direction = daily["wind_direction_10m_dominant"].as_array().ok_or_else(|| anyhow!("Missing wind direction data"))?;
        let empty_vec_prob = Vec::new();
        let precip_prob = daily["precipitation_probability_max"].as_array().unwrap_or(&empty_vec_prob);
        let empty_vec_rain = Vec::new();
        ...
        let sunrise_times = daily["sunrise"].as_array().ok_or_else(|| anyhow!("Missing sunrise data"))?;
        let sunset_times = daily["sunset"].as_array().ok_or_else(|| anyhow!("Missing sunset data"))?;
        
        let mut forecasts = Vec::new();
        
        for i in 0..dates.len().min(7) { // Limit to 7 days (1 week)
            ...
            let sunrise_str = sunrise_times.get(i).and_then(|v| v.as_str()).unwrap_or_default();
            let sunset_str = sunset_times.get(i).and_then(|v| v.as_str()).unwrap_or_default();
    ...
    pub fn get_weather_description_from_wmo(&self, code: u32, is_day: bool) -> WeatherDescription {
        let (main, description, icon) = match code {
            ...
            2 => ("Clouds", "Partly cloudy", if is_day { "03d" } else { "03n" }),
