...
impl LocationService {
    ...
    pub async fn get_location_by_name(&self, location_name: &str) -> Result<Location> {
        ...
        if let Some(place) = json.as_array().and_then(|arr| arr.first()) {
            let lat = place["lat"].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
            let lon = place["lon"].as_str().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
            let name = place["display_name"].as_str().unwrap_or("Unknown").to_string();
            
            // Get more details using reverse geocoding
            return self.get_detailed_location(lat, lon, Some(name)).await;
        }
        
        Err(anyhow::anyhow!("Could not find location: {}", location_name))
    }
    
    /// Get detailed location info from coordinates
    async fn get_detailed_location(&self, lat: f64, lon: f64, name_override: Option<String>) -> Result<Location> {
        let url = format!(
    ...
    fn parse_location_from_json(&self, json: Value) -> Option<Location> {
        let latitude = json["lat"]
            ...
            .or_else(|| json["location"].as_object().and_then(|loc| loc["lat"].as_f64()))?;
            
        let longitude = json["lon"]
            .as_f64()
            .or_else(|| json["longitude"].as_f64())
            .or_else(|| json["location"].as_object().and_then(|loc| loc["lng"].as_f64()))?;
