use crate::modules::types::{WeatherCondition, HourlyForecast, DailyForecast};
use ratatui::{
    layout::Rect,
    style::Color,
    widgets::{Block, Borders},
    widgets::canvas::{Canvas, Context, Line, Points, Circle, Rectangle},
    Frame,
};
...
fn draw_sky_gradient(ctx: &mut Context, is_day: bool, temperature: f64, condition: &WeatherCondition) {
    let layers = 25;
    ...
    for layer in 0..layers {
        ...
        let color = if is_day {
            match condition {
                WeatherCondition::Thunderstorm => {
                    if intensity < 0.3 { Color::Black }
                    else if intensity < 0.7 { Color::DarkGray }
                    else { Color::Gray }
                }
                WeatherCondition::Fog | WeatherCondition::Mist => {
                    if intensity < 0.5 { Color::Gray }
                    else { Color::White }
                }
                _ => {
                    if temperature > 35.0 {
                        if intensity < 0.2 { Color::Yellow }
                        else if intensity < 0.5 { Color::LightYellow }
                        else if intensity < 0.8 { Color::LightBlue }
                        else { Color::Blue }
                    } else if temperature < 5.0 {
                        if intensity < 0.3 { Color::White }
                        else if intensity < 0.7 { Color::LightBlue }
                        else { Color::Blue }
                    } else {
                        if intensity < 0.4 { Color::LightBlue }
                        else if intensity < 0.8 { Color::Blue }
                        else { Color::Blue }
                    }
                }
            }
            ...
            match condition {
                WeatherCondition::Clear => {
                    if intensity < 0.6 { Color::Black }
                    else if intensity < 0.9 { Color::Blue }
                    else { Color::Blue }
                }
                _ => {
                    if intensity < 0.8 { Color::Black }
                    else { Color::DarkGray }
                }
            }
        };
...
fn draw_ground_terrain(ctx: &mut Context, condition: &WeatherCondition) {
    ...
    for y in 0..50 {
        let base_color = match condition {
            ...
            _ => if y > 30 { Color::Green } else { Color::DarkGray },
        };
    ...
    for ray in 0..24 {
        ...
        for t in 0..thickness {
            ...
            ctx.draw(&Line {
                ...
                color: if t < thickness / 2 { sun_color } else { Color::Yellow },
            });
        }
    }
...
fn draw_beautiful_moon(ctx: &mut Context, x: f64, y: f64) {
    ...
    for (cx, cy, cr, color) in craters.iter() {
        ...
        ctx.draw(&Circle {
            x: cx - 0.5, y: cy - 0.5, radius: cr + 0.8,
            color: Color::White,
        });
    }
...
fn draw_stellar_field(ctx: &mut Context) {
    let constellations = [
        // Big Dipper pattern
        [(80.0, 170.0), (90.0, 175.0), (100.0, 172.0), (115.0, 170.0), 
         (125.0, 165.0), (130.0, 160.0), (120.0, 155.0)],
        // Orion pattern
        [(300.0, 180.0), (310.0, 175.0), (320.0, 170.0), (315.0, 160.0),
         (305.0, 165.0), (325.0, 165.0), (330.0, 155.0)],
    ];
    ...
    for constellation in constellations.iter() {
        ...
        for i in 0..constellation.len() - 1 {
            ctx.draw(&Line {
                x1: constellation[i].0, y1: constellation[i].1,
                x2: constellation[i + 1].0, y2: constellation[i + 1].1,
                color: Color::DarkGray,
            });
        }
    ...
    let scattered_stars = [
        (50.0, 185.0, 2.0), (150.0, 180.0, 2.5), (200.0, 190.0, 2.0),
        (250.0, 185.0, 3.0), (350.0, 175.0, 2.5), (370.0, 190.0, 2.0),
        (20.0, 165.0, 1.5), (180.0, 160.0, 2.0), (280.0, 195.0, 2.5),
    ];
...
fn draw_twinkling_star(ctx: &mut Context, x: f64, y: f64, size: f64) {
    ...
    let ray_points = [
        (x - size, y), (x + size, y),
        (x, y - size), (x, y + size),
        (x - size * 0.7, y - size * 0.7), (x + size * 0.7, y + size * 0.7),
        (x - size * 0.7, y + size * 0.7), (x + size * 0.7, y - size * 0.7),
    ];
...
fn draw_realistic_cloud(ctx: &mut Context, x: f64, y: f64, size: f64, base_color: Color, _opacity: f64, is_storm: bool) {
...
fn draw_rain_system(ctx: &mut Context, heavy_rain: bool, wind_speed: f64) {
    ...
    for i in 0..drop_density {
        for layer in 0..25 {
            ...
            let y_pos = ((layer * fall_speed + animation_offset as usize + i * 2) % 140 + 60) as f64;
            ...
            if final_x >= 0.0 && final_x < 400.0 && y_pos > 50.0 {
                ...
                ctx.draw(&Line {
                    x1: final_x, y1: y_pos,
                    x2: final_x + wind_lean * 0.3, y2: drop_bottom,
                    color: Color::Blue,
                });
                
                // Raindrop highlight
                ctx.draw(&Line {
                    x1: final_x + 0.5, y1: y_pos,
                    x2: final_x + 0.5 + wind_lean * 0.3, y2: drop_bottom,
                ...
                if y_pos < 65.0 {
                    let splash_points = [
                        (final_x - 3.0, 50.0), (final_x + 3.0, 50.0),
                        (final_x - 1.5, 48.0), (final_x + 1.5, 48.0),
                        (final_x, 46.0),
                    ];
                    ctx.draw(&Points {
...
fn draw_puddles(ctx: &mut Context) {
    let puddles = [
        (70.0, 42.0, 35.0), (180.0, 45.0, 40.0), (290.0, 44.0, 30.0), (360.0, 43.0, 25.0)
    ];
...
fn draw_lightning_bolt(ctx: &mut Context, start_x: f64, start_y: f64) {
    ...
    for (i, (x, y)) in main_path.iter().enumerate().skip(1).step_by(2) {
        ...
        ctx.draw(&Line {
            x1: *x, y1: *y,
            x2: branch_end_x, y2: branch_end_y,
            color: Color::LightYellow,
        });
        ...
        // Sub-branches
        ctx.draw(&Line {
            x1: branch_end_x, y1: branch_end_y,
            x2: branch_end_x + 8.0, y2: branch_end_y - 8.0,
            color: Color::Yellow,
        });
    }
            ...
            if final_x >= 0.0 && final_x < 400.0 && y_pos > 50.0 {
                ...
                for thickness in 0..4 {
                    ctx.draw(&Line {
                        ...
                        y2: drop_bottom,
                        color: if thickness < 2 { Color::Blue } else { Color::LightBlue },
                    });
                }
            }
        }
...
fn draw_storm_ground_effects(ctx: &mut Context) {
    ...
    let storm_puddles = [
        (80.0, 40.0, 50.0), (200.0, 43.0, 60.0), (320.0, 41.0, 45.0)
    ];
    ...
    match flake_type {
        ...
        1 => {
            ...
            for i in 0..6 {
                let next_i = (i + 1) % 6;
                ctx.draw(&Line {
                    x1: hex_points[i].0, y1: hex_points[i].1,
                    x2: hex_points[next_i].0, y2: hex_points[next_i].1,
                    color: Color::White,
                });
            }
        ...
        2 => {
            ...
            for &(end_x, end_y) in main_arms.iter() {
                ...
                for branch in 1..3 {
                    ...
                    ctx.draw(&Points {
                        coords: &[(branch_x + 1.0, branch_y + 1.0), (branch_x - 1.0, branch_y - 1.0)],
                        color: Color::White,
                    });
                }
            }
    ...
    for layer in 0..layers {
        ...
        for x in (0..400).step_by(layer_density) {
            ...
            if fog_opacity > 0.3 {
                ...
                ctx.draw(&Line {
                    x1: x as f64, y1: final_y,
                    x2: (x + layer_density) as f64, y2: final_y,
                    color: fog_color,
                });
    ...
    for stream in 0..num_streams {
        ...
        for segment in 0..25 {
            ...
            if moving_x < 400.0 {
                ...
                // Main wind line
                ctx.draw(&Line {
                    x1: moving_x, y1: wave_y,
                    x2: end_x.min(400.0), y2: end_y,
                    color: wind_color,
                });
        ...
        for h in 0..grass_height {
            ...
            if grass_x >= 0.0 && grass_x < 400.0 {
                ctx.draw(&Points {
                    ...
                    color: if h > grass_height / 2 { Color::Green } else { Color::DarkGray },
                });
            }
        }
    }
...
fn draw_weather_indicators(ctx: &mut Context, temperature: f64, humidity: u8, wind_speed: f64) {
    ...
    for segment in 0..wind_strength {
        ...
        ctx.draw(&Line {
            x1: flag_x, y1: segment_y,
            x2: flag_x + flag_length, y2: segment_y,
            color: Color::Red,
        });
    }
    ...
    if let Some(current) = hourly_data.first() {
        ...
        let canvas = Canvas::default()
            ...
            .paint(|ctx| {
                ...
                let warning_points = [
                    (200.0, 90.0), (190.0, 110.0), (210.0, 110.0)
                ];
                for i in 0..warning_points.len() {
                    let next_i = (i + 1) % warning_points.len();
                    ctx.draw(&Line {
                        x1: warning_points[i].0, y1: warning_points[i].1,
                        x2: warning_points[next_i].0, y2: warning_points[next_i].1,
                        color: Color::Red,
                    });
                }
            ...
            for (i, day) in daily_data.iter().take(7).enumerate() {
                ...
                match day.main_condition {
                    ...
                    WeatherCondition::Clouds => {
                        ...
                        draw_realistic_cloud(ctx, x_offset + 30.0, 65.0, 8.0, Color::White, 1.0, false);
                    }
                    WeatherCondition::Rain | WeatherCondition::Drizzle => {
                        // Rain cloud with animation
                        draw_realistic_cloud(ctx, x_offset + 30.0, 70.0, 7.0, Color::Gray, 1.0, false);
                    ...
                    WeatherCondition::Thunderstorm => {
                        ...
                        draw_realistic_cloud(ctx, x_offset + 30.0, 70.0, 8.0, Color::DarkGray, 1.0, true);
                        ...
                        for i in 0..lightning_points.len() - 1 {
                            ctx.draw(&Line {
                                x1: lightning_points[i].0, y1: lightning_points[i].1,
                                x2: lightning_points[i + 1].0, y2: lightning_points[i + 1].1,
                                color: Color::LightYellow,
                            });
                        }
                    }
                    WeatherCondition::Snow => {
                        // Snow cloud
                        draw_realistic_cloud(ctx, x_offset + 30.0, 70.0, 7.0, Color::White, 1.0, false);
                        
                        // Mini snowflakes
                        let snow_positions = [
                            (x_offset + 25.0, 55.0), (x_offset + 28.0, 50.0),
                            (x_offset + 32.0, 58.0), (x_offset + 35.0, 52.0),
                            (x_offset + 30.0, 45.0), (x_offset + 26.0, 48.0),
                        ];
                    ...
                    WeatherCondition::Fog | WeatherCondition::Mist => {
                        ...
                        for layer in 0..5 {
                            ...
                            ctx.draw(&Line {
                                ...
                                x2: x_offset + 50.0,
                                y2: fog_y,
                                color: if layer % 2 == 0 { Color::Gray } else { Color::White },
                            });
                        }
                    }
                    _ => {
                        ...
                        ctx.draw(&Line {
                            x1: x_offset + 27.0, y1: 62.0,
                            x2: x_offset + 30.0, y2: 60.0,
                            color: Color::White,
                        });
                        ctx.draw(&Line {
                            x1: x_offset + 30.0, y1: 60.0,
                            x2: x_offset + 33.0, y2: 62.0,
                            color: Color::White,
                        });
                        ctx.draw(&Line {
                            x1: x_offset + 33.0, y1: 62.0,
                            x2: x_offset + 30.0, y2: 65.0,
                            color: Color::White,
                        });
                        ctx.draw(&Points {
                ...
                if i < 6 {
                    ctx.draw(&Line {
                        x1: x_offset + 70.0, y1: 0.0,
                        x2: x_offset + 70.0, y2: 100.0,
                        color: Color::DarkGray,
                    });
